...
function Statistique(){
  ...
  // if(DELETEevent){
  //   var newTotalEvent= Number(totalevent.textContent);
  //   var newToatlSeats=Number(toatlseats.textContent);
  //   var newTotalRevenu=Number(totalrevenu.textContent);
...
function DELETEevent(btn) {
  // const delett = btn.closest('.table__row');
  const index=btn-1;
  // const id=delett.dataset.eventId;
...
function restorEvent(btnn){
  //  const indexx = btn.closest('.table__row');
  const index=btnn - 1;
  // const id=indexx.dataset.eventId;
  // const index=events.findIndex(e=>e.id===id);
